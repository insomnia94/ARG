...
class DavisOneshotDataset(OneshotImageDataset):
  def __init__(self, config, subset, use_old_label):
    ...
    #list_file = "ImageSets/480p/trainsplit" + str(self.trainsplit) + "_val.txt" if self.trainsplit > 0 else "ImageSets/480p/val.txt"
    list_file = NAME_PAIR_PATH
    #list_file = "name_pair/train_parent_test"
    #self._video_tags, self._videos = self.load_videos(self.data_dir + list_file, self.data_dir, video_range=video_range)
