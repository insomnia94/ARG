...
class Critic(object):
    ...
    def save(self):
        with self.sess.as_default():
            with self.graph.as_default():
                self.saver.save(self.sess, "./critic_models/critic_model.ckpt")

    def restore(self):
        with self.sess.as_default():
            with self.graph.as_default():
                self.saver.restore(self.sess, "./critic_models/critic_model.ckpt")

    def save_learning_rate(self):
        f = open("./critic_models/critic_lr", "w")
        f.write(str(self.lr))

    def restore_learning_rate(self):
        f = open("./critic_models/critic_lr", "r")
