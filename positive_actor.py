...
class Positive_Actor(object):
    ...
    def save(self):
        with self.sess.as_default():
            with self.graph.as_default():
                self.saver.save(self.sess, "./positive_actor_models/actor_model.ckpt")

    def restore(self):
        with self.sess.as_default():
            with self.graph.as_default():
                self.saver.restore(self.sess, "./positive_actor_models/actor_model.ckpt")

    def save_learning_rate(self):
        f = open("./positive_actor_models/actor_lr", "w")
        f.write(str(self.lr))

    def restore_learning_rate(self):
        f =  open("./positive_actor_models/actor_lr", "r")
